...
using ICELauncher.Core.Net.MojangApi.Responses;
...
namespace ICELauncher.Core.Net.MojangApi.Endpoints
{
    ...
    public class Statistics : IEndpoint<StatisticsResponse>
    {
        ...
        public async override Task<StatisticsResponse> PerformRequestAsync()
        {
            ...
            statistics.PostContent = "{ \"metricKeys\": [" + string.Join(",", statistics.Arguments.ConvertAll(x => $"\"{x.ToString()}\"").ToArray()) + "]}";
            statistics.Response = await Requester.Post(statistics);

            if (statistics.Response.IsSuccess)
            {
                JObject stats = JObject.Parse(statistics.Response.RawMessage);

                return new StatisticsResponse(statistics.Response)
                {
            ...
            {
                if (statistics.Response.Code == HttpStatusCode.BadRequest)
                {
                    return new StatisticsResponse(new Response(statistics.Response)
                    {
                        Error =
                        {
                    ...
                    return new StatisticsResponse(Error.GetError(statistics.Response));
            }
        }
